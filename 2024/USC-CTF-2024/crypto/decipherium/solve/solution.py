# Manually define a dictionary of elements with their atomic numbers
periodic_table = {
...
# Step 1: Extract element symbols from the message
elements = []
i = 0
while i < len(encoded_message):
    # Check for two-letter or one-letter element symbol
    if i + 1 < len(encoded_message) and encoded_message[i:i+2] in periodic_table:
        ...
        i += 1  # Skip unknown symbol for safety

# Step 2: Convert atomic numbers to ASCII characters if in range
decoded_text = ""
for symbol in elements:
    atomic_number = periodic_table[symbol]
    # Check if the atomic number corresponds to a printable ASCII character
    if 32 <= atomic_number <= 126:
        decoded_text += chr(atomic_number)
    else:
        decoded_text += "?"  # Placeholder for unprintable characters
